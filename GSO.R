...
GSO = function(X){
	if(is.matrix(X)){
		...
		count = 0
		u = X[,1]
		while((dotpro(u,u)==0)&&(count<n-1)) #Loop for ignoring initial zero columns (if any)
		{
			u = X[,count + 2]
			count = count + 1
		}
		if(dotpro(u,u)==0){
			...
			if(count<n-1){
				...
				for(i in (count + 2):n)
				{
					...
					w = u
					lv = length(v)
					for(j in 1:(lv/m)) #Loop for elimination of projection on previous column vectors
					{
						for(k in 1:m)
						{
							sv[k] = v[(j-1) * m + k]
					...
					if(dotpro(w,w)<=0.00000001*dotpro(u,u)){
...
print('Input:')
print("GSO(matrix(c(1,2,3,4,5,6,7,8,10),3,3))")
print('Output:')
GSO(matrix(c(1,2,3,4,5,6,7,8,10),3,3))